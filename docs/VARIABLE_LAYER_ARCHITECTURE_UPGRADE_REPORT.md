...
同一业务事实在原模板出现多次时，系统只匹配和审核一次，再同步到所有物理位置。变量数量表示“待回填事实”，不表示实体数量；同一实体的姓名、职务和证书号仍是不同事实。
...
- 默认值必须由受控企业事实或项目角色绑定自动取得，业务人员以审核为主；如果自动结果确实有误，可在 Word 预览中作仅当前项目生效的留痕修正，不反写企业知识库。
...
- 前端显示“N 个待回填事实覆盖 M 个原模板位置”；可展开查看每个原始位置、上下文和证据。
- 用户可选择已核验候选人并建立项目角色，不需要重复填写同一事实。
- 已绑定实体的合并键是“实体 + 属性”：同一人即使在不同章节被称为法定代表人、签署人，其姓名也只生成一个待回填事实。
- 两个不同实体即使文字值恰好一样，也不会错误合并。
- 右侧可点击“在预览中定位”，左侧将跳转到对应表格单元格或段落并高亮；可在预览中作本项目人工修正，修正结果按变量同步且不改写企业知识库。
- 用户可见内容不展示 `custom_*`、UUID、内部来源代码或数据库字段。

## 5. 验证结果

- 变量字典、槽位去重、角色隔离、实体事实收敛、内部标识隐藏和预览人工修正：8 项新测试通过。
- Entity Resolution / Generation Profile / Response Template 聚焦回归合计 64 项通过；后端全量 337 项通过。
- 真实自贡 DOCX：识别 151 个槽位候选，其中 137 个为本次必填位置；必填位置收敛为 109 个待回填事实，分布于 17 个实体/对象范围，合并 28 个重复必填位置。其余 14 个“可选/如有”位置保留在模板描述中但不冒充必填项；人员表按“人员行 + 属性”保持分离，绑定真实 `person_id` 后再跨章节收敛。
- Python `compileall` 与本次变更的 `pyflakes` 检查通过。
- 前端静态测试 2/2 和 production build 通过。浏览器使用合成脱敏 Word 与隔离 API 完成“上传 → 右侧定位 → 左侧表格高亮 → 修改 → 保存 → Word 预览刷新”，命中单元格从“张三”更新为“李四”，控制台零错误且无 `custom_*`/UUID 泄漏；真实后端保存回路仍需在迁移 026 获授权并重建本地服务后复核。
- 模型兼容 SDK 改为真实调用时懒加载；不调用模型的核心服务导入由约 73 秒降至约 0.46 秒，不改变模型路由与调用行为。
