...
- 模型仅在规则无法高置信处理时使用，供应商可恢复错误最多切换三次。
...
写作运行，默认最多 3 次调用、180,000 Token；这既避免长招标文件挤占章节额度，
也避免章节重试无限消耗。反向代理为受控章节请求保留 300 秒响应窗口，生成成功
后再返回并持久化版本。

模型切换策略位于 `config/rules/model_routing.default.json`。只有额度不足、限流、
模型未开通、超时或供应商 5xx 等可恢复错误才会按任务切换备用模型，最多尝试
3 个不同模型；业务校验、输出格式或代码错误不会触发模型切换。
