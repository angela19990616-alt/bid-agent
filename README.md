...
严格回填先由本地解析器发现表格、冒号、下划线等物理空位，再由 AI 语义识别层按
章节、整句、表头、行列和相邻文字批量判断业务概念；AI 只判断“要什么”，不提供
任何企业值。判断结果必须通过版本化 Ontology 的实体、角色、属性和值类型校验，
低置信、冲突或越界输出保留为待审核，不会直接写入文件。明显且高置信的标准字段
沿用本地 Ontology 结果，避免重复模型调用；同一规则版本的 AI 结果持久化复用。

语义通过后执行 Entity Resolution + Role Binding：每个空位保存章节、
